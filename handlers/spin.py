...
async def cmd_spin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if cooldown_time > 0:
        ...
        if update.message:
            ...
            await update.callback_query.edit_message_text(msg_text, parse_mode="HTML", reply_markup=back_home_kb())
        return
...
async def cb_spin_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for frame in frames:
        try:
            await update.callback_query.edit_message_text(frame, parse_mode="HTML")
    ...
    try:
        await update.callback_query.edit_message_text(result_text, parse_mode="HTML", reply_markup=back_home_kb())
    except Exception:
        pass
