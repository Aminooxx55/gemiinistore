...
async def notify_admin(context: ContextTypes.DEFAULT_TYPE, text: str):
    ...
    try:
        await context.bot.send_message(chat_id=ADMIN_ID, text=text, parse_mode="MarkdownV2")
    except Exception:
        pass
...
async def process_order_delivery(db, bot, order_id: int):
    ...
    if cashback_amount > 0:
        ...
        cashback_str = escape_md(f"${cashback_amount:.2f}")
        try:
            await bot.send_message(
                chat_id=user_id,
                text=f"🎁 *Loyalty Cashback Awarded\\!*\n\nYou earned {cashback_str} cashback for Order \\#{order_id}\\. It has been added to your wallet\\! 💰",
    ...
    if delivery_successful:
        ...
        try:
            msg = (
                f"📦 *Order \\#{order_id} Delivered Instantly\\!*\n\n"
                f"Thank you for your purchase\\! Your product credentials / details are below:\n\n"
                f"`{escape_md(delivery_info)}`\n\n"
